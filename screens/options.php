...
{
	register_setting('mra_sitepush_options', 'mra_sitepush_options', 'mra_sitepush_validate_options');

	/* General settings fields */
	add_settings_section(
		'mra_sitepush_section_config',
		'General Configuration',
		'mra_sitepush_section_config_text',
		'sitepush_options'	
	);
	...
	add_settings_section(
		...
		'Cache management',
		'mra_sitepush_section_cache_text',
		'sitepush_options'	
	);
	add_settings_field(
	...
	add_settings_section(
		...
		'Plugin management',
		'mra_sitepush_section_plugins_text',
		'sitepush_options'	
	);

	add_settings_field(
		'mra_sitepush_field_plugin_management',
		'Plugin management',
		'mra_sitepush_field_plugin_management',
		'sitepush_options',
		'mra_sitepush_section_plugins'
...
function mra_sitepush_section_config_text()
{
...
function mra_sitepush_section_plugins_text()
{
	...
	foreach( $mra_sitepush_options['plugins']['activate'] as $name=>$path )
	{
		$name = is_numeric( $name ) ? $path : $name;
		$activates .= "<li>{$name}</li>";
	}
	
	foreach( $mra_sitepush_options['plugins']['deactivate'] as $name=>$path )
	{
		$name = is_numeric( $name ) ? $path : $name;
...
function mra_sitepush_field_sites_conf()
{
	echo mra_get_sitepush_input_text('sites_conf','','large-text');
}

function mra_sitepush_field_dbs_conf()
{
	echo mra_get_sitepush_input_text('dbs_conf','','large-text');
}

function mra_sitepush_field_backup_path()
{
	echo mra_get_sitepush_input_text('backup_path','If you leave this blank, destination site will not be backed up before a push.','large-text');
}

function mra_sitepush_field_timezone()
{
	echo mra_get_sitepush_input_text('timezone','Your default timezone is  <i>' . date_default_timezone_get() . '</i>. If that is not correct, enter your timezone here to make sure that logs and reporting are in your correct local time. See <a href="http://php.net/manual/en/timezones.php" target="_blank">list of supported timezones</a> for valid values.');
}

function mra_sitepush_field_capability()
{
	echo mra_get_sitepush_input_text('capability');
}

function mra_sitepush_field_admin_capability()
{
	echo mra_get_sitepush_input_text('admin_capability');
}

function mra_sitepush_field_cache()
{
	$caches = array(
			  'w3tc' => 'W3 Total Cache'
			, 'none' => 'None <span class="description">(select this if you have a cache installed but it is not listed above)</span>'
	);
	echo mra_get_sitepush_input_radio('cache', $caches);
}
function mra_sitepush_field_cache_key()
{
	global $mra_sitepush_options;
	
	$extra_text = empty( $mra_sitepush_options['cache_key'] ) ? "<br />A random string you could use: " .  md5( microtime() ) : '';

	echo mra_get_sitepush_input_text('cache_key', "A hard to guess secret key. This ensures that the cache is only cleared on a destination site when you want it to. This key must be the same on all sites which you push to from this site.{$extra_text}");
}

function mra_sitepush_field_plugin_management()
{
	//echo mra_get_sitepush_input_text('admin_capability');
}

// actual HTML creator
function mra_get_sitepush_input_text( $field, $description='', $class='regular-text' )
{
	global $mra_sitepush_options;
	$value = empty( $mra_sitepush_options[$field] ) ? '' : $mra_sitepush_options[$field];
	$output = "<input id='mra_sitepush_field_{$field}' name='mra_sitepush_options[{$field}]' type='text' value='{$value}' class='{$class}' />";
	if( $description )
		$output .= "<span class='description' style='display:block;'>{$description}</span>";
	return $output;
}

function mra_get_sitepush_input_radio( $field, $radio_options, $description='' )
{
	global $mra_sitepush_options;
	...
	{
		$output .= "<label><input name='mra_sitepush_options[{$field}]' type='radio' value='{$radio_option}'" . checked($radio_option, $mra_sitepush_options[$field], FALSE) . " /> {$label}</label><br />\n";
	}
